...
import { Form, redirect , Await ,defer, useLoaderData,  useNavigation, useAsyncError, useRouteError} from 'react-router-dom'
...
function User() {
   ...
   useEffect(()=>{
    ...
    const CartData =(vanData)=>{
          ...
          const Content = cart.map(ele =>{
           ...
           return  <div key={ele.id} className='mt-2 flex p-4 
  ...
  return (
    <div className='mt-20 mb-24'>
        ...
        <p> {email}</p>
        <button disabled={formStatus === "submitting"} type='submit' className={`${formStatus === "idle" ? "bg-orange-600"
        :"bg-orange-300"}
        w-full p-2  rounded-full text-[20px] text-white`}>
        {formStatus === "idle" ?"Log Out": "Logging Out" }</button>
        </div>
        </Form>
    ...
    <div className='w-full pb-6 pt-2 min-h-[150px] px-6 mt-6 rounded-md bg-orange-300'>
      <div className='  justify-between font-semibold
        ...
        <div className='mt-10 '>
        <Suspense  fallback={
<BarLoader className='w-fit mx-auto' width={window.innerWidth -300} color="#FF5F1F" />}>
          <Await errorElement={ <AsyncErrorElement/>} resolve={cartDataPromise}>
