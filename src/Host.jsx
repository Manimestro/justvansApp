...
function HostHeader() {
  return (
    <div className="w-fit flex flex-row sm:gap-10 gap-4 md:text-[18px] mt-1  ">
      {NAV_ITEMS.map((ele) => (
        <NavLink
          end={ele.id === "."}
          key={ele.id}
          ...
          className={({ isActive }) =>
            isActive
              ? "text-[20px] font-bold underline cursor-pointer "
...
do {
  star.push(
    <svg
      ...
      viewBox="0 0 24 24"
      fill="currentColor"
      className="w-6 text-amber-500 h-6"
    >
      <path
        fillRule="evenodd"
      ...
      <div className="mt-4">
        <div className="mt-16 flex flex-col gap-10 w-11/12  ">
          <Suspense fallback={<Loading />}>
            <Await
              errorElement={<NoHostVansError />}
              resolve={promisedvans.data}
            >
              {(resolvedVans) => {
                ...
                return vans.map((van) => {
                  ...
                  return (
                    <div
                      key={van.id}
                      className="flex rounded-lg  bg-white p-4 flex-row items-center min-w-[150px]   "
                    >
                      <Link
                        to={`${van.id}`}
                        className="flex w-11/12 flex-row gap-4 min-w-[150px]   "
                      >
                        <img
                        ...
                        />
                        <div className="">
                          <p className="text-[25px]  font-bold w-fit">
                            {vanData.name}
                          </p>
                          <p className=" text-[20px] w-fit ">
                            &#8377;{vanData.price}/day
                          </p>
                        </div>
                      </Link>
...
function Income() {
  return (
    <div className="mt-16 w-11/12   ">
      <img src={img} className="w-full max-w-[700px]"></img>
    </div>
  );
}
...
async function loader() {
  ...
  const CustumPromise = new Promise(async (resolve, reject) => {
    const mail = localStorage.getItem("email");
