...
export default function () {
  return (
    <nav className="flex  flex-wrap justify-between">
      <Link to={"."}>
        <h1 className="font-bold text-[26px] uppercase">#JustVans</h1>
      </Link>
    ...
    <main className=" w-fit flex flex-row    ">
      ...
      >
        ...
        <motion.div
          ...
          className="fixed top-0 left-0  h-screen gap-10 w-full flex felx-col  mobile-menu"
        >
          <motion.button
          ...
          >
            {MOBILE_NAV_ITEMS.map((navItem) => (
              <motion.li whileTap={{ scale: 0.95 }} key={navItem.id}>
                <motion.div
                  variants={liVariant}
                  className="text-[20px] hover:italic w-fit mx-auto"
                  onClick={() => setMobileNavOpen(!mobileNavOpen)}
                >
                  <Link to={navItem.id}>{navItem.navTitle}</Link>
                </motion.div>
              </motion.li>
            ))}
          </motion.ul>
